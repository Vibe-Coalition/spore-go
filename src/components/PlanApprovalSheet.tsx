      ...
      <Modal transparent animationType="slide">
        <View style={s.overlay}>
          <View style={[s.sheet, { backgroundColor: t.bgPanel, borderColor: t.border }]}>
            <Text style={{ color: t.fg, fontSize: 14, marginBottom: 12 }}>
              ┌─ revision feedback {'─'.repeat(14)}┐
            </Text>
            <TextInput
              style={{ color: t.fg, fontSize: 13, borderWidth: 1, borderColor: t.border, backgroundColor: t.bgInput, padding: 10, minHeight: 80, textAlignVertical: 'top' }}
        ...
        <View style={[s.sheet, { backgroundColor: t.bgPanel, borderColor: t.border }]}>
          ...
          {hasFiles && (
            <View style={{ borderWidth: 1, borderColor: t.border, padding: 10, marginBottom: 12 }}>
              <Text style={{ color: t.accent, fontSize: 11, marginBottom: 6 }}>files affected:</Text>
              <ScrollView style={{ maxHeight: 120 }}>
                {summary.creates.map(f => (
                  <Text key={f} style={{ color: t.success, fontSize: 12 }}>  + {f}</Text>
                ))}
                {summary.modifies.map(f => (
                  <Text key={f} style={{ color: t.warning, fontSize: 12 }}>  ~ {f}</Text>
                ))}
              </ScrollView>
            </View>
          )}
          ...
          <TouchableOpacity style={[s.btn, { borderColor: t.success }]} onPress={onExecute}>
            <Text style={{ color: t.success, fontSize: 14 }}>execute plan</Text>
          </TouchableOpacity>

          <TouchableOpacity style={[s.btn, { borderColor: t.accent }]} onPress={() => setShowFeedback(true)}>
            <Text style={{ color: t.accent, fontSize: 14 }}>revise with feedback</Text>
          </TouchableOpacity>
...
const s = StyleSheet.create({
  ...
  sheet: { borderWidth: 1, borderTopLeftRadius: 18, borderTopRightRadius: 18, padding: 16, paddingBottom: Platform.OS === 'ios' ? 36 : 16 },
